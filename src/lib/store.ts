...
import type { AppData, Ingredient, Product, Recipe, FairEvent, Transaction, Sale, PaymentMethod, EventCost, ProductionGoal, RecipeIngredient } from "@/types";
...
async function loadDataFromSupabase(userId: string) {
  try {
    const [ingRes, prodRes, recRes, riRes, evtRes, salesRes, txRes, pgRes] = await Promise.all([
    ...
    data = { ingredients, products, recipes, events, transactions, productionGoals };
...
export async function registerSale(eventId: string, productId: string, paymentMethod: PaymentMethod, quantity: number = 1): Promise<Sale | undefined> {
  const event = data.events.find(e => e.id === eventId);
  ...
  const { data: inserted, error } = await db.from('sales').insert({
    ...
    quantity, unit_price: product.price, total, payment_method: paymentMethod,
...
export async function undoSale(eventId: string, saleId: string) {
  ...
  const updatedProducts = (event.products || []).map((ep: any) =>
    ep.productId === sale.productId ? { ...ep, quantitySold: Math.max(0, ep.quantitySold - sale.quantity) } : ep
  );
  await db.from('events').update({ products: updatedProducts as any }).eq('id', eventId);

  const product = data.products.find(p => p.id === sale.productId);
  if (product) {
    await db.from('products').update({ quantity: product.quantity + sale.quantity }).eq('id', sale.productId);
  }
