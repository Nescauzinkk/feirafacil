...
// Lock to prevent race conditions: concurrent sales of the same product
// would read stale in-memory state, causing stock to not be properly deducted.
const saleInProgress = new Set<string>();
...
async function loadDataFromSupabase(userId: string) {
  try {
    ...
    const products: Product[] = (prodRes.data || []).map((r: any) => ({
      ...
      priceRegional: r.price_regional != null ? Number(r.price_regional) : undefined,
  ...
  try {
    ...
    for (const prod of old.products || []) {
      const { data: inserted } = await db.from('products').insert({
        ...
        price_regional: prod.priceRegional ?? null,
        resale_price: prod.resalePrice ?? null,
        quantity: prod.quantity, category: prod.category,
      }).select('id').single();
...
export async function addProduct(prod: Omit<Product, 'id'>): Promise<Product> {
  const { data: inserted, error } = await db.from('products').insert({
    ...
    price_regional: prod.priceRegional ?? null,
    resale_price: prod.resalePrice ?? null,
    quantity: prod.quantity, recipe_id: prod.recipeId, category: prod.category,
  }).select().single();
  ...
  const item: Product = {
    ...
    priceRegional: inserted.price_regional != null ? Number(inserted.price_regional) : undefined,
    resalePrice: inserted.resale_price != null ? Number(inserted.resale_price) : undefined,
...
export async function updateProduct(id: string, updates: Partial<Product>) {
  ...
  if (updates.priceRegional !== undefined) dbUpdates.price_regional = updates.priceRegional ?? null;
  if (updates.resalePrice !== undefined) dbUpdates.resale_price = updates.resalePrice ?? null;
  if (updates.quantity !== undefined) dbUpdates.quantity = updates.quantity;
...
export async function registerSale(eventId: string, productId: string, paymentMethod: PaymentMethod, quantity: number = 1, customUnitPrice?: number): Promise<Sale | undefined> {
  // --- ANTI RACE-CONDITION ---
  // Sem este lock, cliques rápidos (ou duplo-clique) leem o mesmo valor de
  // estoque da memória antes de qualquer atualização ser concluída, fazendo
  // com que cada venda calcule "product.quantity - quantity" com o mesmo ponto
  // de partida. O resultado é que o estoque nunca diminui corretamente —
  // ou em casos extremos, cresce (daí o "multiplica" relatado pela cliente).
  const lockKey = `${eventId}:${productId}`;
  if (saleInProgress.has(lockKey)) {
    throw new Error('Venda em andamento para este produto. Aguarde.');
  }
  saleInProgress.add(lockKey);

  try {
    const event = data.events.find(e => e.id === eventId);
    if (!event) return undefined;

    // Lê a quantidade atual DIRETO DO BANCO para evitar estoque stale em memória
    const { data: freshProduct, error: fetchError } = await db
      .from('products').select('id, name, unit, price, price_regional, resale_price, quantity, recipe_id, category')
      .eq('id', productId).single();
    if (fetchError || !freshProduct) return undefined;

    const currentStock = Number(freshProduct.quantity);
    if (currentStock < quantity) {
      throw new Error(`Estoque insuficiente: disponível ${currentStock}, solicitado ${quantity}`);
    }

    const unitPrice = customUnitPrice ?? Number(freshProduct.price);
    ...
    const sale: Sale = {
      ...
      unitPrice, total, paymentMethod,
      timestamp: inserted.sold_at, day: inserted.day,
    };

    // Atualiza quantitySold no JSON do evento usando o evento atual em memória
    const updatedEventProducts = (event.products || []).map((ep: any) =>
      ep.productId === productId ? { ...ep, quantitySold: (ep.quantitySold || 0) + quantity } : ep
    );
    await db.from('events').update({ products: updatedEventProducts as any }).eq('id', eventId);

    // Debita estoque com o valor lido DO BANCO (não da memória)
    ...
    const { data: txInserted } = await db.from('transactions').insert({
      ...
      description: `Venda: ${freshProduct.name} (${quantity} ${freshProduct.unit || 'un'})`,
      category: 'Vendas', date: new Date().toISOString(), event_id: eventId,
    }).select().single();

    // Atualiza memória de uma só vez (snapshot atômico)
    data = {
      ...data,
      events: data.events.map(e => {
        if (e.id !== eventId) return e;
        return { ...e, sales: [...e.sales, sale], products: updatedEventProducts };
      }),
      products: data.products.map(p => p.id === productId ? { ...p, quantity: newQty } : p),
      transactions: txInserted
        ? [...data.transactions, {
            id: txInserted.id, type: 'income' as const, amount: total,
...
export async function undoSale(eventId: string, saleId: string) {
  ...
  // Lê quantidade atual do banco antes de devolver para evitar estado stale
  const { data: freshProduct } = await db.from('products').select('quantity').eq('id', sale.productId).single();
  const currentQty = freshProduct ? Number(freshProduct.quantity) : (data.products.find(p => p.id === sale.productId)?.quantity || 0);
  const newQty = currentQty + sale.quantity;
  await db.from('products').update({ quantity: newQty }).eq('id', sale.productId);

  // Remove transação relacionada — usa saleId para identificar com mais precisão
  const relatedTx = data.transactions.find(t =>
    t.eventId === eventId && t.type === 'income' && t.category === 'Vendas' && t.amount === sale.total &&
    t.description.includes(data.products.find(p => p.id === sale.productId)?.name || '')
  );
  if (relatedTx) {
  ...
  data = {
    ...
    events: data.events.map(e => {
      if (e.id !== eventId) return e;
      return { ...e, sales: e.sales.filter(s => s.id !== saleId), products: updatedProducts };
    }),
    products: data.products.map(p => p.id === sale.productId ? { ...p, quantity: newQty } : p),
