...
function AddTransactionForm({ type, onClose, editTx }: { type: 'income' | 'expense'; onClose: () => void; editTx?: { id: string; amount: number; description: string; category: string } }) {
  ...
  const updateTotalAmount = (products: { productId: string; quantity: number }[]) => {
    let total = 0;
    products.forEach(p => {
      const prod = data.products.find(item => item.id === p.productId);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editTx) {
        await updateTransaction(editTx.id, { amount: Number(amount), description, category: category || 'Outros' });
      } else {
      ...
      // toast is imported in Finance via sonner
  ...
  return (
    <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2">
      ...
      {type === 'income' && (
        <div className="space-y-2 border p-3 rounded-lg bg-gray-50/50">
          <div className="flex items-center justify-between">
            <Label className="text-xs font-bold uppercase text-muted-foreground">Produtos (Opcional)</Label>
            <Button variant="ghost" size="sm" onClick={addProduct} className="h-7 text-xs text-green-700">
          ...
          {selectedProducts.map((p, index) => (
            <div key={index} className="flex gap-2 items-end">
              <div className="flex-1">
                <Select value={p.productId} onValueChange={(val) => updateProduct(index, val)}>
                  <SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Produto" /></SelectTrigger>
                  <SelectContent className="bg-white">
                    {data.products.map(prod => (
                      <SelectItem key={prod.id} value={prod.id}>{prod.name} (R$ {prod.price})</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div className="w-20">
                <Input type="number" min="1" value={p.quantity} onChange={(e: any) => updateQuantity(index, Number(e.target.value))} className="h-9 text-xs" />
              </div>
              <Button variant="ghost" size="icon" onClick={() => removeProduct(index)} className="h-9 w-9 text-destructive">
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
          {selectedProducts.length === 0 && (
            <p className="text-[10px] text-muted-foreground italic text-center">Nenhum produto selecionado. O estoque não será alterado.</p>
          )}
        </div>
      )}
      ...
      <Button onClick={handleSubmit} className="w-full bg-green-700 hover:bg-green-800 text-white" size="lg" disabled={loading}>
        {loading ? 'Salvando...' : (editTx ? 'Atualizar' : 'Salvar')}
      </Button>
    </div>
  );
}
