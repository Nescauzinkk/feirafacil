...
import type { PaymentMethod, EventProduct, EventCost, ProductGoal, DayGoals, PriceType } from "@/types";
    ...
    <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Button type="button" className={`h-12 text-xs font-bold ${
            priceType === 'local'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <Button className="flex-1 bg-green-700 hover:bg-green-800 text-white" onClick={handleSubmit}>
...
function EventSalesMode({ eventId }: { eventId: string }) {
  ...
  const [saleLoading, setSaleLoading] = useState<string | null>(null); // productId em processamento
  const event = data.events.find(e => e.id === eventId);

  if (!event) return <p>Evento não encontrado</p>;

  const priceType = event.priceType || 'local';
  ...
  const handleSale = async (productId: string, quantity: number = 1) => {
    if (saleLoading) return; // bloqueia se já há venda em andamento
    const prod = data.products.find(p => p.id === productId);
    if (!prod) return;

    // Verifica estoque em memória antes de tentar (o banco valida de verdade)
    const ep = event.products.find(p => p.productId === productId);
    const remaining = ep ? ep.quantityTaken - ep.quantitySold : 0;
    if (remaining <= 0) {
      toast.error(`${prod.name} sem estoque no evento`);
      return;
    }

    let unitPrice = prod.price;
    if (priceType === 'regional' && prod.priceRegional) unitPrice = prod.priceRegional;
    else if (priceType === 'resale' && prod.resalePrice) unitPrice = prod.resalePrice;

    setSaleLoading(productId);
    try {
      const sale = await registerSale(event.id, productId, paymentMethod, quantity, unitPrice);
      if (sale) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-sm border-green-700 text-green-700">
            {priceType === 'local' ? 'Local' : priceType === 'regional' ? 'Regional' : 'Revenda'}
          </Badge>
          <Badge variant="destructive" className="animate-pulse text-white bg-red-500">AO VIVO</Badge>
        </div>
      </div>
        ...
        <>
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="p-5 text-center">
              ...
              <p className="text-4xl font-extrabold text-green-700 text-primary">R$ {totalSold.toFixed(2)}</p>
              {isMultiDay && (
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <Button variant={paymentMethod === 'cash' ? 'default' : 'outline'} className={`h-14 flex-col gap-1 text-xs font-bold ${paymentMethod === 'cash' ? 'bg-green-700 hover:bg-green-800 text-white' : 'border-2 border-green-700 text-green-700 hover:bg-green-50'}`} onClick={() => setPaymentMethod('cash')}>
                <Banknote className="h-4 w-4" />Dinheiro
              </Button>
              <Button variant={paymentMethod === 'pix' ? 'default' : 'outline'} className={`h-14 flex-col gap-1 text-xs font-bold ${paymentMethod === 'pix' ? 'bg-green-700 hover:bg-green-800 text-white' : 'border-2 border-green-700 text-green-700 hover:bg-green-50'}`} onClick={() => setPaymentMethod('pix')}>
                <Smartphone className="h-4 w-4" />Pix
              </Button>
              <Button variant={paymentMethod === 'card' ? 'default' : 'outline'} className={`h-14 flex-col gap-1 text-xs font-bold ${paymentMethod === 'card' ? 'bg-green-700 hover:bg-green-800 text-white' : 'border-2 border-green-700 text-green-700 hover:bg-green-50'}`} onClick={() => setPaymentMethod('card')}>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {event.products.map(ep => {
                ...
                const currentPrice = priceType === 'regional' && prod.priceRegional ? prod.priceRegional : (priceType === 'resale' && prod.resalePrice ? prod.resalePrice : prod.price);
                const isThisLoading = saleLoading === ep.productId;
                const isAnyLoading = !!saleLoading;
                return (
                  <Button
                    key={ep.productId}
                    variant="outline"
                    className={`h-28 flex-col gap-2 text-lg font-bold border-2 transition-all active:scale-95 ${remaining > 0 && !isAnyLoading
                      ? 'border-primary bg-primary/15 hover:bg-primary/25 text-primary'
                      : remaining <= 0
                        ? 'border-destructive/30 bg-destructive/5 opacity-60'
                        : 'opacity-70 cursor-not-allowed'
                      }`}
                    disabled={remaining <= 0 || isAnyLoading}
                    onClick={() => {
                      if (isFractional) {
                  ...
                  >
                    ...
                    <span className="text-sm font-normal opacity-70">
                      R$ {currentPrice.toFixed(2)}/{UNIT_LABELS[prod.unit || 'un']} • {remaining % 1 === 0 ? remaining : remaining.toFixed(2)} restante
                    </span>
                  </Button>
                );
              })}
            ...
            <DialogContent>
              ...
              {saleQtyDialog && (() => {
                ...
                return (
                  <div className="space-y-4">
                    {(() => {
                      const currentPrice = priceType === 'regional' && prod?.priceRegional ? prod.priceRegional : (priceType === 'resale' && prod?.resalePrice ? prod.resalePrice : (prod?.price || 0));
                      return (
                        <>
                          <p className="text-muted-foreground">{prod?.name} — R$ {currentPrice.toFixed(2)}/{UNIT_LABELS[prod?.unit || 'un']}</p>
                          ...
                          {Number(saleQty) > 0 && prod && (
                            <p className="text-sm font-semibold">Total: R$ {(Number(saleQty) * currentPrice).toFixed(2)}</p>
                          )}
                        </>
                      );
                    })()}
                    <Button onClick={handleFractionalSale} className="w-full" size="lg" disabled={!!saleLoading}>
                      {saleLoading ? 'Registrando...' : 'Registrar venda'}
                    </Button>
                  </div>
                );
              })()}
          ...
          {currentDaySales.length > 0 && (
            <Button variant="outline" className="w-full" onClick={handleUndo} disabled={!!saleLoading}>
                ...
                {[...currentDaySales].reverse().slice(0, 10).map(sale => {
                  ...
                  return (
                    <div key={sale.id} className="flex items-center justify-between text-sm py-1">
                      ...
                      <Button variant="ghost" size="sm" className="h-6 px-2 text-xs text-destructive" disabled={!!saleLoading} onClick={() => {
          ...
          {isMultiDay && !allDaysCompleted ? (
            <Button variant="secondary" size="lg" className="w-full text-lg py-6 mt-4" onClick={() => setShowEndDayConfirm(true)} disabled={!!saleLoading}>
              <Sun className="mr-2 h-5 w-5" />Finalizar dia {event.currentDay}
            </Button>
          ) : (
            <Button variant="destructive" size="lg" className="w-full bg-red-500 hover:bg-red-600 text-white text-lg py-6 mt-4" onClick={() => setShowEndEventConfirm(true)} disabled={!!saleLoading}>
      ...
      <Dialog open={showEndEventConfirm} onOpenChange={setShowEndEventConfirm}>
        <DialogContent>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <Button variant="destructive" className="flex-1 bg-green-700 hover:bg-green-800 text-white" onClick={handleEndEvent}>Encerrar</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function EventPreStart({ event }: { event: typeof useStore extends () => infer R ? R extends { events: (infer E)[] } ? E : never : never }) {
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-4xl">
      ...
      <Button size="lg" className="w-full bg-green-700 hover:bg-green-800 text-white text-lg py-6" onClick={() => setShowPrep(true)}>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <Button variant="destructive" className="flex-1 bg-red-500 hover:bg-red-600 text-white" onClick={confirmDelete}>Excluir</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
