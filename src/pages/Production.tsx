...
import { Plus, ChefHat, CheckCircle, XCircle, Trash2, Pencil, ArrowRight, Target, PartyPopper, Share2, ShoppingCart } from "lucide-react";
...
function ProduceDialog({ recipeId, onClose }: { recipeId: string; onClose: () => void }) {
  ...
  const handleAddToList = async () => {
    if (qty > 0) {
      await addToShoppingList(recipeId, qty);
      toast.success("Adicionado à Lista de Mercado!");
      onClose();
    }
  };
          ...
          {check.enough ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-destructive">
                <div className="flex items-center gap-2"><XCircle className="h-5 w-5" /><span>Falta ingrediente:</span></div>
                <Button variant="ghost" size="sm" onClick={handleAddToList} className="h-7 text-[10px] bg-green-100 text-green-700 hover:bg-green-200 border border-green-200">
                  <ShoppingCart className="h-3 w-3 mr-1" /> Adicionar à Lista de Mercado
                </Button>
              </div>
              {check.missing.map(m => (
